...
void Lcd_Draw_MAP(int x, int y, const unsigned short int *fp, unsigned short int *mp)
{
	register int width = fp[0];
	register int xx, yy;
	register int h0=mp[0], h1=mp[1];

	for(yy=0;yy<h1-30;yy++)
	{
		for(xx=0;xx<h0;xx++)
		{
			mp[(yy)*h0+(xx)+2]=fp[(yy+y)*width+(xx+x)+2];
		}
	}
}
void Lcd_Draw_BOSS(int x, int y, const unsigned short int *fp,unsigned short int *tmp, unsigned short int *mp,int color)
{
	register int width = fp[0], height = fp[1];
	register int xx, yy;
	register int h0 = tmp[0];
	register int a0=mp[0],a1=mp[1];

	for(yy=0;yy<a1;yy++)
	{
		for(xx=0;xx<a0;xx++)
		{
			tmp[(yy+60)*h0+(xx+60)+2]=mp[(yy)*a0+(xx)+2];
		}
	}
	for(yy=0;yy<height;yy++)
	{
		for(xx=0;xx<width;xx++)
		{
			if(fp[(yy)*width+(xx)+2]!=color)
				tmp[(yy+y+60)*h0+(xx+x+60)+2]=fp[(yy)*width+(xx)+2];
		}
	}
	for(yy=0;yy<a1;yy++)
	{
		for(xx=0;xx<a0;xx++)
		{
			mp[(yy)*a0+(xx)+2]=tmp[(yy+60)*h0+(xx+60)+2];
		}
	}
}
void Lcd_Draw_Lazer(int x, int y, const unsigned short int *fp,unsigned short int *tmp, unsigned short int *mp,int color)
{
	register int width = fp[0], height = fp[1];
	register int xx, yy;
	register int h0 = tmp[0];
	...
	for(yy=0;yy<height;yy++)
	{
		for(i=0;i<15;i++)
		{
			for(xx=0;xx<width;xx++)
			{
				tmp[(yy+y+60+30)*h0+(xx+x+60+60*i+10)+2]=fp[(yy)*width+(xx)+2];
			}
		}
	}
	for(yy=0;yy<a1;yy++)
...
void Lcd_Draw_Char(int x, int y, const unsigned short int *fp, unsigned short int *mp,int color)
{
	register int width = fp[0], height = fp[1];
	register int xx, yy;
	register int h0 = mp[0];
	...
	for(yy=0;yy<height;yy++)
	{
		...
		{
			if(fp[(yy)*width+(xx)+2]!=color)
				mp[(yy+y)*h0+(xx+x)+2]=fp[(yy)*width+(xx)+2];
		}
	}
}
void Lcd_Show_MAP(unsigned short int *fp, unsigned short int *mp)
{
	register int width = fp[0], height = fp[1];
	register int xx, yy;

	for(yy=0;yy<height-30;yy++)
	{
		for(xx=0;xx<width;xx++)
		{
//			if(fp[(yy)*width+(xx)+2]!=mp[(yy)*width+(xx)+2])
